// tests/skills/org-map/helpers.mjs — test helpers for /org-map.
// (Kept inside tests/skills/org-map/ rather than the shared tests/helpers/.)
...
// Gate values.
//
// `skills.org_map` is MERGED into _lib/gates.yaml now, so `loadGates()` is the
// working state rather than the pending one. This suite still may not edit that file,
// so what it needs from here is the mirror image of what it needed before the merge:
//
//   1. REQUESTED_GATES stops being a request and becomes a PIN. This suite's
//      arithmetic (four pages, a 25-profile committee, a 200-row batch floor) is
//      calibrated to these numbers; a silent edit in gates.yaml must be a red run
//      rather than a quiet change of behaviour. `inference-and-gates.test.mjs`
//      asserts the shipped file resolves to exactly these, which keeps the pin honest.
//   2. `gatesWithout()` strips the block back out. Now that the keys exist that is
//      the only honest way to keep testing the fail-closed half — a test that relied
//      on the real file lacking the key stopped testing anything the moment it landed.
// ---------------------------------------------------------------------------
...
export function gatesWithRequestedKeys (base) {
  ...
  gates.skills.org_map = { ...REQUESTED_GATES.org_map };
  return gates;
}
...
export function tmpRoot (prefix = 'org-map-') {
 ...
 */
export function validatorSandbox (skillNames, { gatesSrc = null } = {}) {
  const dir = tmpRoot('org-map-validate-');
  ...
  if (gatesSrc !== null) writeFileSync(join(dir, '_lib', 'gates.yaml'), gatesSrc);
