// tests/skills/call-intel/skill-shape.test.mjs
//
...
import { loadCallIntelRules, CallIntelRulesUnavailable, SKILL_PATH } from './harness.mjs';
...
test('rule 1 — `## Related` is present and every relative link resolves', () => {
  ...
  assert.ok(links.some(l => l.includes('/account-research/')),
    'the account context a call brief sits inside must be routed to');
  assert.ok(links.some(l => l.includes('/comply/')),
    'erasure of a transcript somebody asks you to remove belongs to /comply');
});
...
test('rule 2 — a boundary section is present and states real ceilings', () => {
  ...
  // The three ceilings a reader of a call-intel skill will otherwise assume it clears.
  assert.match(flat, /will not record, dial, or transcribe/i,
    'there is no telephony and no speech recognition in this pack, permanently');
  assert.match(flat, /sending is deliberately external/i,
    'the pack\'s permanent ceiling must be stated, not implied');
  assert.match(flat, /will not invent an objection/i,
...
test('rule 4 — every cited gate key resolves against the real gates.yaml', () => {
  ...
  assert.ok(cited.length >= 3,
    'this skill has two optional paid hops; the gates that fire inside them must be named');
  ...
  for (const key of ['session_budget.fractions.confirm', 'session_budget.fractions.stop']) {
    assert.ok(cited.includes(key), `${key} is not cited — the gates that fire inside the run must be named`);
  }
});
...
test('the gate keys this skill reads are merged, and are cited properly', () => {
  ...
  for (const key of ['skills.call_intel.ai_enrich_batch_min_transcripts', 'skills.call_intel.max_enrich_profile_calls_per_run']) {
...
test('the skill never hand-rolls a paid call — everything goes through the runtime', () => {
  ...
  assert.ok(!/api\.richapi|https?:\/\/[^\s)]*\/(?:ai_enrich|enrich_profile)/i.test(BODY),
...
test('the CLI verbs and flags this skill instructs actually exist in bin/richapi', () => {
  ...
  for (const invented of ['richapi transcribe', 'richapi call-intel', 'richapi summarise', 'richapi extract']) {
...
test('there is exactly one call-intel-rules block, and it parses', () => {
  ...
  assert.equal(typeof loadCallIntelRules(), 'object');
});

test('the Iron Law is stated where a reader cannot miss it, and named as the Iron Law', () => {
  // Not a prose assertion about wording: this checks the law has its own heading rather
  // than living in a bullet halfway down a step. A rule nobody reads is not a rule.
  assert.ok(HEADINGS.some(h => /iron law/i.test(h)), 'the Iron Law needs its own section');
  const flat = BODY.replace(/\s+/g, ' ');
  assert.match(flat, /A transcript with no objection must not produce an objection/i);
});
...
test('the real validator passes on this skill in an isolated sandbox, with zero warnings', () => {
  ...
  assert.ok(population.includes('account-research') && population.includes('richapi-gtm'),
    'this skill must route onward to the account brief and the router');
