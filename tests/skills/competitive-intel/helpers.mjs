// tests/skills/competitive-intel/helpers.mjs — test helpers.
// (Kept inside tests/skills/competitive-intel/ rather than the shared tests/helpers/.)
...
export const SKILL = 'competitive-intel';
...
// Gate values.
//
// `skills.competitive_intel` is MERGED into _lib/gates.yaml now, so `loadGates()` is
// the working state rather than the pending one. This suite still may not edit that
// file, so what it needs from here is the mirror image of what it needed before:
//
//   1. REQUESTED_GATES stops being a request and becomes a PIN. This suite's sweep
//      arithmetic is calibrated to these numbers, so a silent edit in gates.yaml has to
//      be a red run rather than a quiet change of what a sweep costs.
...
export function gatesWithRequestedKeys (base) {
  ...
  gates.skills.competitive_intel = { ...REQUESTED_GATES.competitive_intel };
  return gates;
}
...
export function tmpRoot (prefix = 'competitive-intel-') {
...
export function validatorSandbox (skillNames, { gatesSrc = null } = {}) {
  const dir = tmpRoot('competitive-intel-validate-');
